...
console.log 자체가 부하가 발생하기 때문에 가급적 기록하는 일은 없어야 하지만, 만약 디버깅 목적으로 사용하고 있다면 환경변수 등의 방법을 사용해 개발자만 제한적으로 볼 수 있게 해야 한다. 

Observed Metics: 최초 바이트까지의 시간, 렌더링 시작에 소요되는 시간, 최초 콘텐츠풀 페인트 등을 측정할 수 있는 다양한 시간 지표에 대해 나난다. 

Individual Runs: 기본적으로 WebPageTest는 3번의 테스트를 돌려서 평균값을 보여주는데, 각 실행별 결과를 확인할 수 있다.
