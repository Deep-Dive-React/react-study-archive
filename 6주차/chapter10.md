...
* 주 버전이 릴리스되면 이전 버전에서의 API 제공을 중단하고 전체 애플리케이션을 새롭게 업그레이드하기를 요구했으나, 이제 17 버전부터는 점진적인 업그레이드가 가능해진다.
...
웹에서 이 두 가지를 보면 '그냥 버튼'은 이벤트 리스너에 click으로 추가돼 있고, '리액트 버튼'은 button의 onClick이벤트에 noop이라는 핸들러가 추가돼 있는 것을 볼 수 있다.  
